rankhospital <- function(state, outcome, num = 'best') { 
  # this function finds the hospital with a given rate of the given outcome in the given state
  ...
  if(outcome=='heart attack'){ #if we concider heart attack
    ...
    if(num=="worst") num<- dim(heart_attack)[1] #if you want the worst I take the last rank
    if(num > dim(heart_attack)[1]) return(NA) # if the num is higher than hihgest rank, function returns NA
    
    
    return(heart_attack[heart_attack[,3]==num, 1]) #I return the name of hospitals with chosen rank
  } 
  ...
  if(outcome=='heart failure'){ #if we concider heart failure
    ...
    if(num=="worst") num<- dim(heart_failure)[1] #if you want the worst I take the last rank
    if(num > dim(heart_failure)[1]) return(NA) # if the num is higher than hihgest rank, function returns NA
  
    
    return(heart_failure[heart_failure[,3]==num,1]) #I return the name of hospitals with the given rank
  }
  ...
  if(outcome=='pneumonia'){ #if we concider pneumonia
    ...
    if(num=="worst") num<- dim(pneumonia)[1] #if you want the worst I take the last rank
    if(num > dim(pneumonia)[1]) return(NA) # if the num is higher than hihgest rank, function returns NA
    
 
    return(pneumonia[pneumonia[,3]==num,1]) #I return the name of hospitals with the given rank
  }
